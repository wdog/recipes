function _tide_item_git_wdog

    if git branch --show-current 2>/dev/null | string shorten -m$tide_git_wdog_truncation_length | read -l location
        git rev-parse --git-dir --is-inside-git-dir | read -fL gdir in_gdir
        set location $location
    else if test $pipestatus[1] != 0
        return
    else if git tag --points-at HEAD | string shorten -m$tide_git_wdog_truncation_length | read location
        git rev-parse --git-dir --is-inside-git-dir | read -fL gdir in_gdir
...
(0|(?<dirty>.*))\n(0|(?<untracked>.*))(\n(0|(?<behind>.*))\t(0|(?<ahead>.*)))?' \
    ...
    if test -n "$operation$conflicted"
        set -g tide_git_bg_color $tide_git_wdog_bg_color_urgent
    else if test -n "$staged$dirty$untracked"
        set -g tide_git_bg_color $tide_git_wdog_bg_color_unstable
    end


    _tide_print_item git_wdog $tide_git_wdog_icon '' (
        set_color white; echo -ns "$location"
        set_color $tide_git_wdog_color_operation; echo -ns ' '$operation ' '$step/$total_steps
        set_color $tide_git_wdog_color_upstream; echo -ns ' ⇣ '$behind ' ⇡ '$ahead
        set_color $tide_git_wdog_color_stash; echo -ns '  '$stash
        set_color $tide_git_wdog_color_conflicted; echo -ns '  '$conflicted
        set_color $tide_git_wdog_color_staged; echo -ns '  '$staged
        set_color $tide_git_wdog_color_dirty; echo -ns '  '$dirty
        set_color $tide_git_wdog_color_untracked; echo -ns '  '$untracked)
end
